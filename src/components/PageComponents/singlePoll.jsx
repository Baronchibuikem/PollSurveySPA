        ...
        <div className="card" style={{ borderColor: "darkblue" }}>
          ...
          <div className="card-body">
            <div className="d-flex justify-content-start">
              {params.single_poll.image !== null ? (
                <img
                  src={params.single_poll.image}
                  alt="poll_creator_image"
                  width="30px"
                  className="mr-3"
              ...
              <h6
                className="card-title mr-5 mt-2 pollhover"
                onClick={() => get_user(params.single_poll.poll_creator_id)}
              >
                {params.single_poll.poll_creator_fullname} @
                {params.single_poll.poll_creator}
              </h6>
            </div>
            <hr />
            <p className="card-text">
              {params.single_poll.poll_question}
              {params.user.user.username === params.single_poll.poll_creator ? (
                <i
                  className="fa fa-trash mt-2 ml-3"
                  style={{ color: "red" }}
                  onClick={handleSubmit(delete_poll)}
                ></i>
              ) : (
                ""
              )}
            ...
            {params.single_poll.poll_has_expired ? (
              <div>
                <span className="text-danger">
                  Voting has ended on this poll choices
                </span>
                {params.single_poll.choices.map((choice) => {
                  return (
                    <button
                      key={choice.id}
                      disabled="disabled"
                      style={defaultColor.disable_button_color}
              ...
              <div>
                {params.single_poll.choices.map((choice) => {
                  return params.user.user.username ===
                    params.single_poll.poll_creator ? (
                    <button
                      key={choice.id}
                      disabled="disabled"
                    ...
                    <button
                      ...
                      key={choice.id}
                      onClick={() =>
                        cast_vote(params.single_poll.id, choice.id)
                      }
                    >
                      {choice.choice_name} {choice.choice_vote_count}
                    </button>
                ...
                params.single_poll.poll_creator ? (
                  <p className="text-danger">You can't vote on your own poll</p>
                ) : (
                  ""
                )}
              </div>
            ...
            <div className="mt-4">
              {get_user_bookmarks().indexOf(
                params.single_poll.poll_question
              ) !== -1 ? (
                <span className="mr-3">
                  <i class="fa fa-book" style={{ color: "#413a76" }}></i>{" "}
                  bookmarked
                </span>
              ) : (
                <span
                  onClick={() =>
                    bookmark_poll(params.single_poll.id, params.user.user.id)
                  }
                  className="pollhover mr-3"
                >
                  <i className="fa fa-book"></i>Bookmark
                </span>
              )}

              {get_user_likes().indexOf(params.single_poll.poll_question) !==
              -1 ? (
                <span className="mr-3">
                  <i className="fa fa-heart" style={{ color: "#413a76" }}></i>
                  Liked ({params.single_poll.total_likes})
                </span>
              ) : (
                <span
                  onClick={() =>
                    like_poll(params.single_poll.id, params.user.user.id)
                  }
                  className="pollhover mr-3"
                >
              ...
              {params.single_poll.vote_count > 0 ? (
                <span style={{ color: "#413a76" }}>
                  Total Votes ({params.single_poll.vote_count})
                </span>
              ) : (
                ""
              )}
